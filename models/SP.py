"""
Augmented Lagrangian Decomposition (ALD) for the 4-stage, 81-scenario salmon
farming MILP defined in ip.py.

The algorithm is a binary PH followed by an extensive-form SLP:
  1. Relax non-anticipativity constraints (NACs) and solve 81 subproblems in parallel.
...
Binary variables are shared only among scenarios that pass through the same node:
  ...
  Stage 0  (@s0)                — shared across all 81 scenarios
  Stage 1  (@s1_{t1})           — shared within 27-scenario stage-1 branches
  Stage 2  (@s2_{t1}_{t2})      — shared within 9-scenario stage-1+2 branches
  Stage 3  (@s3_{t1}_{t2}_{t3}) — shared within 3-scenario stage-1+2+3 branches
...
class BinaryProgressiveHedging:
    """
    Solves the 81-scenario salmon farming MILP via Augmented Lagrangian
    ...
    def __init__(
        ...
        *,
        ...
        self.slam_force_fix_stall   = slam_force_fix_stall
    ...
    def build(self):
        """Build all 81 scenario models and the NA variable indexing structures."""
        T       = self.T
        labels  = self.labels
        n_total = len(labels) ** 4   # 81
        temp_map = {
            "bad":    self.temps_bad,
        ...
        for l1 in labels:
            for l2 in labels:
                for l3 in labels:
                    for l4 in labels:
                        name    = f"s1_{l1}__s2_{l2}__s3_{l3}__s4_{l4}"
                        temps_t = np.zeros(T)
    ...
    def _build_variable_index(self):
        ...
        information available at that decision stage:
          - Stage-0 vars: "@s0"            (all 81 scenarios share one x̄)
          - Stage-1 vars: "@s1_{l1}"       (27 scenarios per node, 3 nodes)
          - Stage-2 vars: "@s2_{l1}_{l2}"  (9 scenarios per node, 9 nodes)
          - Stage-3 vars: "@s3_{l1}_{l2}_{l3}" (3 scenarios per node, 27 nodes)
        """
        labels      = self.labels
        ...
        for sc_idx, milp in enumerate(self.milp_objects):
            l1 = labels[sc_idx // (n_labels ** 3)]
            l2 = labels[(sc_idx // (n_labels ** 2)) % n_labels]
            l3 = labels[(sc_idx // n_labels) % n_labels]
            ...
            #   s0              — blind commit, l1 unknown          (1 node,  81 scenarios)
            #   s1_{l1}         — l1 observed, l2 unknown           (3 nodes, 27 scenarios each)
            #   s2_{l1}_{l2}    — l2 observed, l3 unknown           (9 nodes,  9 scenarios each)
            #   s3_{l1}_{l2}_{l3} — l3 observed, l4 unknown        (27 nodes, 3 scenarios each)
            _add_stage("s0",                    self.stage0_months)
            _add_stage(f"s1_{l1}",              self.stage1_months)
            _add_stage(f"s2_{l1}_{l2}",         self.stage2_months)
            _add_stage(f"s3_{l1}_{l2}_{l3}",    self.stage3_months)
        ...
        for sc_idx in range(n_scenarios):
            ...
            _node_scens[f"s3_{l1}_{l2}_{l3}"].append(sc_idx)
    ...
    def solve(self):
        ...
        while k < K and not converged:
            ...
            # Tail-stagnation escape: if disagreement is flat for many iterations,
            # force-fix the remaining binary NA vars to rounded x_bar.
            # Primary trigger: disagree <= tail_fix_max_disagree and flat.
            # Fallback trigger: slam has been active >= slam_force_fix_stall iters
            #   and disagree has been perfectly flat — handles tight-MAB cases where
            #   many variables are structurally split (xbar≈0.5) and never cycle.
            tail_fixed = 0
            _post_slam_iters = (k - _slam_triggered_at) if (_slam_triggered and _slam_triggered_at is not None) else 0
            if (
                not converged
                and _slam_triggered
                and bin_disagree > 0
                ...
                _flat = min(recent_disagree) == max(recent_disagree)
                _small_and_flat  = _flat and bin_disagree <= self.tail_fix_max_disagree
                _post_slam_stall = _flat and _post_slam_iters >= self.slam_force_fix_stall
                if _small_and_flat or _post_slam_stall:
                    rem_bin = np.where(is_binary & ~self.fixed)[0]
                    if rem_bin.size > 0:
                        trigger_label = "post-slam-stall" if _post_slam_stall else "small-and-flat"
                        print(
                            f"  [Iter {k}] tail stall detected [{trigger_label}]: "
                            f"disagree={bin_disagree} flat for {self.tail_fix_window} iters "
                            f"(slam active {_post_slam_iters} iters) -> force-fixing {rem_bin.size} vars",
                            flush=True,
                        )
                    for vi in rem_bin:
    ...
    def _post_SLP(self, executor):
        ...
        def _node_key(sc_idx: int, ss: int) -> str:
            l1 = self.labels[sc_idx // (_n_labels ** 3)]
            l2 = self.labels[(sc_idx // (_n_labels ** 2)) % _n_labels]
            l3 = self.labels[(sc_idx // _n_labels) % _n_labels]
            if ss < self.stage0_end:
                return "s0"
            elif ss < self.stage1_end:
                return f"s1_{l1}"
            ...
            if not feasible:
                ...
                with self._progress_lock:
                    if not getattr(self, '_iis_written', False):
                        ...
                        iis_file = f"iis_scenario_{sc_idx}.ilp"
                        model.write(iis_file)
                        tqdm.write(f"  [post-proc sc={sc_idx}] INFEASIBLE — IIS written to {iis_file}")
    ...
    def plot(self, filename: str = "stats/sp_convergence.png"):
        ...
        def _draw_timeline(ax, rep_nodes, title, shade_color):
            ...
            ax.text((_s3_start + T) / 2, _n_units - 0.15, "stage-3",
            ...
            for _u in _units_all:
                ...
                # Terminal cohorts: stocked but not harvested within horizon
        ...
        _draw_timeline(
            axes[0, 0],
            rep_nodes={"s0", "s1_bad", "s2_bad_bad", "s3_bad_bad_bad"},
            title="Bad Path  (s0 → s1_bad → s2_bad_bad → s3_bad_bad_bad)",
            shade_color="tab:red",
        )
        _draw_timeline(
            axes[0, 1],
            rep_nodes={"s0", "s1_normal", "s2_normal_normal", "s3_normal_normal_normal"},
            title="Normal Path  (s0 → s1_normal → s2_normal_normal → s3_normal_normal_normal)",
            shade_color="steelblue",
        )
        _draw_timeline(
            axes[0, 2],
            rep_nodes={"s0", "s1_good", "s2_good_good", "s3_good_good_good"},
            title="Good Path  (s0 → s1_good → s2_good_good → s3_good_good_good)",
        ...
        ax.text(0.5, 0.5,
                ...
                f"--- Tree-aware NA ---\n"
                f"s0: 81 scens | s1: 27/node | s2: 9/node | s3: 3/node\n"
...
def export_solution_bundle(ald: BinaryProgressiveHedging,
    ...
    long_rows = []
    for sc_idx, milp in enumerate(ald.milp_objects):
        sname   = ald.scenario_names[sc_idx]
    ...
    obj_rows = []
    for sc_idx, model in enumerate(ald.scenarios):
        feasible = model.SolCount > 0
        obj_rows.append({
            "scenario_id":   sc_idx,
            "scenario_name": ald.scenario_names[sc_idx],
...
def export_decision_tree(ald: "BinaryProgressiveHedging",
                         filename: str = "stats/st_sp_decision_tree.xlsx"):
    """
    ...
    for sc_idx, name in enumerate(ald.scenario_names):
        ...
        lookup_rows.append({
            ...
            "feasible":      "YES" if sc_idx not in ald.infeasible_scenarios else "NO",
    ...
    guide_rows = [
        ...
        {"Sheet": "Scenario_Playbook",
         "What it shows": "Full chronological action list for every scenario across all 4 stages. "
                          "All decisions are non-anticipative and follow the consensus policy from Tree_Plan.",
         "How to use": "Filter by scenario_id to see the exact sequence of Stock / Harvest actions "
                       "for a given scenario across all four stages."},
    ]
...
if __name__ == "__main__":
    ...
    ald.build()
    ald.solve()
    # export_solution_bundle(ald, outdir="stats/solution_exports_st_sp")
    # export_decision_tree(ald, filename="stats/sp_decision_tree.xlsx")
    ald.print_results()

    n_cages = len(ald.units_df)
    summary = (
        f"\n{n_cages}\n"
        f"{'=' * 70}\n"
        f"Status     : {ald.n_feasible}/{ald.n_scenarios} feasible\n"
        f"Solve time : {ald.total_time:.1f}s\n"
        f"Obj value  : {ald.eval_obj:,.2f}\n"
        f"MIP gap    : {ald.mip_gap:.2%}\n"
        f"{'=' * 70}\n"
    )
    print(summary)
    with open("results_SP.txt", "a") as f:
        f.write(summary)

    ald.plot("stats/sp_convergence_full.png")