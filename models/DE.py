...
Scenario tree (4 stages of uncertainty, T = 60 months):
  Stage 0 (months  0-14): 1  node                  — all 81 scenarios share one plan
  Stage 1 (months 15-29): 3  nodes  (t1)            — 27 scenarios each
  Stage 2 (months 30-44): 9  nodes  (t1,t2)         —  9 scenarios each
  Stage 3 (months 45-59): 27 nodes  (t1,t2,t3)      —  3 scenarios each
Total: 3^4 = 81 scenarios.
...
Stage-0 has 1 node: t1 is NOT known when months 0–14 decisions are made.
Stage-3 has 27 nodes: t3 is known but t4 is NOT yet known when months 45–59
decisions are made, so scenarios sharing (t1,t2,t3) must commit to one plan.
"""
...
class DeterministicEquivalent:
    """Deterministic equivalent (extensive form) for 81-scenario 4-stage salmon farming."""
    ...
    def __init__(
        ...
        *,
        ...
        # Stage boundaries — 4 evenly spaced 15-month stages
        self.stage0_months = list(range(0, 15))
        self.stage1_months = list(range(15, 30))
        self.stage2_months = list(range(30, 45))
        self.stage3_months = list(range(45, T))

        # Temperature patterns (matching sp.py)
        days = 30
        ...
        # Survival — warm water (good temp) raises mortality, matching sp.py.
        # High mortality (S_bad) only when this AND the preceding stage are both "good".
        ...
        self.scenario_s2:     list = []   # stage-1 node: t2 index (0/1/2)
        self.scenario_s3:     list = []   # stage-2 node: t3 index (0/1/2)
    ...
    def _make_scenario_data(self):
        """Build one SalmonFarmingMILP per scenario (for precomputed bpf/biomass_exist/wpf/wpf_exist).
        Sub-models are NOT solved; only their trajectory arrays and structural sets are used."""
        temp_map = {
            "bad":    self.temps_bad,
            "normal": self.temps_normal,
            "good":   self.temps_good,
        }
        for t1 in self.stage_labels:
            for t2 in self.stage_labels:
                for t3 in self.stage_labels:
                    for t4 in self.stage_labels:
                        ...
                        stage_seq = [
                            (t1, self.stage0_months), (t2, self.stage1_months),
                            (t3, self.stage2_months), (t4, self.stage3_months),
                        ]
                        ...
                        self.scenario_bundle.append(self.stage_labels.index(t1))
                        self.scenario_s2.append(self.stage_labels.index(t2))
                        self.scenario_s3.append(self.stage_labels.index(t3))

        self.n_scenarios = len(self.milp_objects)
        self.probs = np.full(self.n_scenarios, 1.0 / self.n_scenarios)
        print(f"  Prepared {self.n_scenarios} scenarios  (3×3×3×3, 4 stages of uncertainty)")
    ...
    def build(self):
        ...
        # ── Structural data (scenario-independent) ────────────────────────────
        ...
        # H_set: union across all 81 scenarios — use the EARLIEST possible harvest time
        # per stocking period across all scenario growth models.  Fish grow fastest under
        # good temperatures, so the union opens harvest windows that bad-scenario models
        # would never allow.  Per-scenario constraints below prohibit harvesting before
        # fish actually reach well_boat_min_weight_g in each specific scenario.
        earliest_ht_union = {
        ...
        # Free Gurobi sub-model objects — DE only needs Python trajectory arrays
        # (bpf, biomass_exist, wpf, wpf_exist), not the Gurobi models.  Disposing them
        # releases the memory / Gurobi resources before building the large DE model.
        for milp in self.milp_objects:
            if milp.model is not None:
                milp.model.dispose()
                milp.model = None

        # prev_x_lt[(u, t)] = [(s, tau)] with tau < t and cohort (u,s) alive at t.
        # Mirrors ip.py's prev_x dict — used in density/MAB/feed-cost.
        ...
        # Per-scenario capacity bound on q (matching ip.py's per_unit_qmax).
        # We use the capacity bound only — NOT the price-threshold tightening —
        # because the threshold is scenario-specific and would cause UB conflicts
        # across NAC-tied scenarios (a cohort unprofitable under bad temps may be
        # profitable under good temps and must be allowed by the NAC decision).
        per_unit_qmax = {}  # key: (sc, u, s)
        for sc in S:
        ...
        # ── Build Gurobi model ────────────────────────────────────────────────
        m = gb.Model("DE_Salmon_81sc")
        m.Params.OutputFlag = 1

        # ── Variables — one set per scenario, matching ip.py types exactly ────
        stk = m.addVars(S, U, Tset, vtype=GRB.BINARY, name="z")

        # q is semi-continuous: either exactly 0 (not stocked) or in [1, UB] (stocked).
        ...
        # x[sc,u,s,t] = harv[sc,u,s,t] * q[sc,u,s]: McCormick linearisation
        x = m.addVars(harv_keys, lb=0.0, name="x")

        # a_new[sc,u,s,t]: alive status of new cohort (u,s) at time t.
        anew_keys = [(sc, u, s, t) for sc in S for (u, s, t) in A_harv_set]
        a_new = m.addVars(anew_keys, lb=0.0, ub=1.0, name="a_new")

        # e_alive: CONTINUOUS [0,1], incremental propagation — always integral.
        ...
        # Soft biomass constraint slack variables
        ...
        # Apply per-scenario bounds on q and stk
        for sc in S:
            for u in U:
                for s in Tset:
        ...
        # ── Per-scenario constraints (matching ip.py _build_model exactly) ────
        for sc in S:
            milp = self.milp_objects[sc]

            # Harvest at most once per cohort (harvest is optional)
            for u in U:
                for s in Tset:
            ...
            # Per-scenario minimum-weight constraint
            for u in U:
                for s in Tset:
            ...
            # Existing cohort harvest (at most once — optional)
            for u in U_exist:
            ...
            # H_exist_biom = biomass_exist[u][t] × h_exist
            for u in U_exist:
                for t in allowed_et:
            ...
            # e_alive incremental propagation
            for u in U_exist:
            ...
            # Unit-level fallow: cannot restock for FALLOW months after any harvest
            for u in U:
                for t in Tset:
                    harvest_terms = []
            ...
            # Location-level mandatory fallow (matching ip.py exactly)
            loc_fallow_start = {
        ...
        stage2_set = set(self.stage2_months)
        stage3_set = set(self.stage3_months)
        ...
        # Stage 0 — 1 node: all 81 scenarios share one plan
        _add_nac(stage0_set, lambda _: 0)
        # Stage 1 — 3 nodes: scenarios sharing t1 agree on months 15–29
        _add_nac(stage1_set, lambda i: self.scenario_bundle[i])
        # Stage 2 — 9 nodes: scenarios sharing (t1,t2) agree on months 30–44
        _add_nac(stage2_set, lambda i: (self.scenario_bundle[i], self.scenario_s2[i]))
        # Stage 3 — 27 nodes: scenarios sharing (t1,t2,t3) agree on months 45–59
        _add_nac(stage3_set, lambda i: (self.scenario_bundle[i],
                                        self.scenario_s2[i],
                                        self.scenario_s3[i]))

        print(f"  Total constraints with NAC: {m.NumConstrs:,}")

        # ── Objective: E[profit] = Σ_sc p_sc × profit_sc ────────────────────
        # Uses weight-class pricing: milp._price_for_kg(weight_kg) looks up the
        # NOK/kg price from the 9-bracket DEFAULT_PRICES table in ip.py.
        ...
        for sc in S:
            ...
            # Revenue — existing cohorts via H_exist_biom
            for u in U_exist:
            ...
            # Terminal value — existing cohorts (via H_exist_biom)
            for u in U_exist:
        ...
        # Penalty for soft biomass constraint violations and early slaughter.
        for sc in S:
    ...
    def print_results(self):
        ...
        normal_sc = next(
            (i for i, nm in enumerate(self.scenario_names)
             if nm == "s1_normal__s2_normal__s3_normal__s4_normal"), 0)
        ...
        _print_stage("Stage-0 Decisions  (months 0-14, all-normal path)",
                     self.stage0_months, normal_sc)
        _print_stage("Stage-1 Decisions  (months 15-29, all-normal path: t1=normal)",
                     self.stage1_months, normal_sc)
        _print_stage("Stage-2 Decisions  (months 30-44, all-normal path: t1=t2=normal)",
                     self.stage2_months, normal_sc)
        _print_stage("Stage-3 Decisions  (months 45-59, all-normal scenario: t1=t2=t3=t4=normal)",
                     self.stage3_months, normal_sc)
    ...
    def plot_biomass(self, output_file: str = "stats/de_biomass.png") -> "plt.Figure":
        ...
        for sc in range(self.n_scenarios):
            ...
            for u in U:
                ...
                unit_biom[sc][u] = b / 1e3  # kg → tonnes
    ...
    def plot(self, output_file: str = "stats/de_solution.png"):
        """
        2×3 figure: Bad / Normal / Good path stocking & harvest timelines (60 months).
        """
        if self.model is None or self.model.SolCount == 0:
        ...
        def _draw_timeline(ax, sc, title, shade_color):
            ...
            for sb, sl in [(15, "S1"), (30, "S2"), (45, "S3")]:
                ax.axvline(sb, color="gray", ls="--", lw=0.8, alpha=0.6)
                ax.text(sb + 0.4, n_units - 0.1, sl,
                        fontsize=6, color="gray", va="top")
            ax.axvspan(s3_start, T, alpha=0.07, color=shade_color, zorder=0)
            ax.text((s3_start + T) / 2, n_units - 0.15, "stage-3",
            ...
            for u in U:
                ...
                if u in U_exist_set:
                    ...
                    for t in range(T):
                        if self._h_exist[sc, u, t].X > 0.5:
                            ...
                            ax.plot(t, y, "v", color="darkorange",
                                    ms=6, zorder=4)
                            harvested = True
                            break
                    if not harvested:
                ...
                for s_start in milp.Tset:
                    if self._stk[sc, u, s_start].X < 0.5:
                        continue
                    ax.plot(s_start, y, "^", color="tab:green",
                            ms=6, zorder=5)
        ...
        bad_sc    = next((i for i, nm in enumerate(self.scenario_names)
                          if nm == "s1_bad__s2_bad__s3_bad__s4_bad"),    0)
        normal_sc = next((i for i, nm in enumerate(self.scenario_names)
                          if nm == "s1_normal__s2_normal__s3_normal__s4_normal"), 40)
        good_sc   = next((i for i, nm in enumerate(self.scenario_names)
                          if nm == "s1_good__s2_good__s3_good__s4_good"), 80)

        _draw_timeline(axes[0, 0], bad_sc,
                       "Bad Path  (s1_bad → s2_bad → s3_bad → s4_bad)",
                       "tab:red")
        _draw_timeline(axes[0, 1], normal_sc,
                       "Normal Path  (s1_normal → s2_normal → s3_normal → s4_normal)",
                       "steelblue")
        _draw_timeline(axes[0, 2], good_sc,
                       "Good Path  (s1_good → s2_good → s3_good → s4_good)",
                       "tab:green")
        ...
        axes[1, 2].text(
            ...
            f"Scenarios  : {self.n_scenarios}  (3⁴, 4 stages of uncertainty)\n"
            ...
            f"--- Scenario Tree ---\n"
            f"Stage-0: 1 node   × 81 scenarios\n"
            f"Stage-1: 3 nodes  × 27 scenarios\n"
            f"Stage-2: 9 nodes  ×  9 scenarios\n"
            f"Stage-3: 27 nodes ×  3 scenarios",
...
def export_decision_tree(de: "DeterministicEquivalent",
    ...
    Sheets: Tree_Plan | Scenario_Lookup | Scenario_Playbook | Guide
    """
    ...
    for sc_idx, name in enumerate(de.scenario_names):
        ...
        nk1 = f"s1_{t1}"
        nk2 = f"s2_{t1}_{t2}"
        nk3 = f"s3_{t1}_{t2}_{t3}"
        sc_nodes.append({"s0": nk0, "s1": nk1, "s2": nk2, "s3": nk3})
        for nk in (nk0, nk1, nk2, nk3):
    ...
    stage2_set = set(de.stage2_months)
    stage3_set = set(de.stage3_months)

    def _months_for_prefix(nk: str) -> set:
        if nk == "s0":             return stage0_set
        if nk.startswith("s1_"):   return stage1_set
        if nk.startswith("s2_"):   return stage2_set
        return stage3_set
    ...
    for sc_idx, name in enumerate(de.scenario_names):
        ...
        nk2 = sc_nodes[sc_idx]["s2"]
        nk3 = sc_nodes[sc_idx]["s3"]
        lookup_rows.append({
            "scenario_id":   sc_idx,
            "t1 (stage0)":   t1,
            "t2 (stage1)":   t2,
            ...
            "s2_stocks":     _count_actions(nk2, "Stock"),
            "s2_harvests":   _count_actions(nk2, "Harvest"),
            "s3_stocks":     _count_actions(nk3, "Stock"),
            "s3_harvests":   _count_actions(nk3, "Harvest"),
            "feasible":      "YES",
    ...
    def _stage_from_month(m: int) -> str:
        ...
        if m in stage1_set: return "Stage1"
        if m in stage2_set: return "Stage2"
        return "Stage3"
            ...
            for t in ref.Tset:
                if round(de._stk[sc_idx, u, t].X) >= 1:
                    ...
                    playbook_rows.append({
                        "scenario_id": sc_idx, "t1": t1, "t2": t2, "t3": t3, "t4": t4,
    ...
    guide_rows = [
        {"Sheet": "Tree_Plan",
         ...
         "How to use": "Read Stage 0 first. After month 14, observe t1 and jump to your Stage-1 branch. "
        ...
        {"Sheet": "Scenario_Lookup",
         "What it shows": "81 rows — one per scenario with node keys and decision counts.",
         "How to use": "Find your scenario by t1/t2/t3/t4 to identify relevant Tree_Plan sections."},
    ...
    _write_stage_banner(ws_tp, row,
                        "STAGE 0  —  Months 0–14",
                        "Committed before any uncertainty resolves.  Identical for ALL 81 scenarios.")
    row += 1
    ...
    _write_stage_banner(ws_tp, row,
                        ...
                        "Decided after observing t2.  Nine branches.")
    row += 1
    for t1 in labels:
        for t2 in labels:
            nk      = f"s2_{t1}_{t2}"
    ...
    _write_stage_banner(ws_tp, row,
                        ...
                        "Decided after observing t3.  27 branches.")
    row += 1
    for t1 in labels:
        for t2 in labels:
            for t3 in labels:
