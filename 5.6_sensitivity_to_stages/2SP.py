"""
Augmented Lagrangian Decomposition (ALD) for the 2-stage, 9-scenario salmon
farming MILP defined in IP.py.

The algorithm is a binary PH followed by an extensive-form SLP:
  1. Relax non-anticipativity constraints (NACs) and solve 9 subproblems in parallel.
...
Binary variables are shared only among scenarios that pass through the same node:
  ...
  Stage 0  (@s0)                — shared across all 9 scenarios
  Stage 1  (@s1_{t1})           — shared within 3-scenario stage-1 branches
...
class AugmentedLagrangianDecomposition:
    """
    Solves the 9-scenario salmon farming MILP via Augmented Lagrangian
    ...
    def __init__(
        ...
        *,
        ...
        self.all_na_names_ordered:       list       = []
        ...
        self._xbar_W:                    np.ndarray = None
        self._participating:             np.ndarray = None
        self._node_scens:                dict       = {}
        self._vi_to_node:                list       = []
        self._vi_occurrences:            list       = []
    ...
    def build(self):
        """Build all 9 scenario models and the NA variable indexing structures."""
        T       = self.T
        labels  = self.labels
        n_total = len(labels) ** 2   # 9
        temp_map = {
            "bad":    self.temps_bad,
    ...
    def _build_variable_index(self):
        ...
        information available at that decision stage:
          - Stage-0 vars: "@s0"       (all 9 scenarios share one x̄)
          - Stage-1 vars: "@s1_{l1}"  (3 scenarios per node, 3 nodes)
        """
        labels      = self.labels
        n_scenarios = self.n_scenarios
        n_labels    = len(labels)  # 3

        na_var_names_per_scenario: list[list[str]] = []

        for sc_idx, milp in enumerate(self.milp_objects):
            l1 = labels[sc_idx // n_labels]
            ...
            #   s0          — blind commit, l1 unknown  (1 node, 9 scenarios)
            #   s1_{l1}     — l1 observed, l2 unknown   (3 nodes, 3 scenarios each)
    ...
    def solve(self):
        ...
        while k < K and not converged:
            ...
            while pending:
                ...
                if done_count != last_progress and (done_count % 3 == 0 or done_count == n_scenarios):
            ...
            diff_uf[~self._participating] = 0.0
            ...
            self.multipliers[~self._participating] = 0.0
    ...
    def _post_SLP(self, executor):
        """Robust q LP across all scenarios, then per-scenario re-solve."""
        n_scenarios = self.n_scenarios
        V           = self.V
        T           = self.T
        probs       = self.probs
        ...
        for vi, qname in enumerate(self.all_na_names_ordered):
            ...
            ss    = int(ss_str)
            # z belongs to the node of time ss; h_nk encodes node of harvest time th.
            if ss < self.stage0_end:
                z_nk = "s0"
            else:
                z_nk = h_nk  # both z and h are at stage-1
        ...
        # ================================================================
        # Two-stage stochastic LP for q (deterministic equivalent with NACs)
        # ================================================================
        ...
        def _node_key(sc_idx: int, ss: int) -> str:
            l1 = self.labels[sc_idx // _n_labels]
            if ss < self.stage0_end:
                return "s0"
            else:
                return f"s1_{l1}"
        ...
        if not rob_feasible:
            ...
            print(f"Two-stage stochastic LP in extensive form solved in {_rob_lp_time:.2f}s. Robust q found.")
    ...
    def print_results(self):
        ...
        print("\n--- Branch differentiation (stage-1 binary consensus) ---")
        ...
        print(f"  Branch-specific binary vars (stage-1): {total_branch_vars}")
    ...
    def plot(self, filename: str = "stats/sp_convergence.png"):
        ...
        T          = self.T
        _BH        = 0.45
        _s1_start  = self.stage1_months[0] if self.stage1_months else T
        ...
        def _draw_timeline(ax, rep_nodes, title, shade_color):
            ...
            ax.axvspan(_s1_start, T, alpha=0.07, color=shade_color, zorder=0)
            ax.text((_s1_start + T) / 2, _n_units - 0.15, "stage-1",
                    ha="center", va="top", fontsize=6, color=shade_color, style="italic")
            ax.axvline(self.stage0_end, color="gray", ls="--", lw=0.8, alpha=0.6)
            ax.text(self.stage0_end + 0.4, _n_units - 0.1, "S1",
        ...
        _draw_timeline(
            axes[0, 0],
            rep_nodes={"s0", "s1_bad"},
            title="Bad Path  (s0 → s1_bad)",
            shade_color="tab:red",
        )
        _draw_timeline(
            axes[0, 1],
            rep_nodes={"s0", "s1_normal"},
            title="Normal Path  (s0 → s1_normal)",
            shade_color="steelblue",
        )
        _draw_timeline(
            axes[0, 2],
            rep_nodes={"s0", "s1_good"},
            title="Good Path  (s0 → s1_good)",
    ...
    def _plot_biomass(self, filename: str = "stats/sp_biomass.png"):
        """
        Plot total regional biomass trajectories across all 9 scenarios.
        """
        n_scenarios = self.n_scenarios
        T = self.T
        probs = self.probs
        ...
        ax.axvline(self.stage0_end, color="purple", ls="--", lw=0.6, alpha=0.3)

        ax.set_xlabel("Month")
        ax.set_ylabel("Total Biomass (tonnes)")
        ax.set_title("Regional Biomass Trajectories (all 9 scenarios)")
