...
Scenario tree (3 stages of uncertainty, T = 60 months):
  Stage 0 (months  0-19): 1  node                  — all 27 scenarios share one plan
  Stage 1 (months 20-39): 3  nodes  (t1)            —  9 scenarios each
  Stage 2 (months 40-59): 9  nodes  (t1,t2)         —  3 scenarios each
Total: 3^3 = 27 scenarios.
...
Stage-0 has 1 node: t1 is NOT known when months 0–19 decisions are made.
Stage-2 has 9 nodes: t2 is known but t3 is NOT yet known when months 40–59
decisions are made, so scenarios sharing (t1,t2) must commit to one plan.
"""
...
class DeterministicEquivalent:
    """Deterministic equivalent (extensive form) for 27-scenario 3-stage salmon farming."""
        ...
        for l1 in self.stage_labels:
            for l2 in self.stage_labels:
                for l3 in self.stage_labels:
                    ...
                    stage_seq = [
                        (l1, self.stage0_months), (l2, self.stage1_months),
                    ...
                    self.scenario_s2.append(self.stage_labels.index(l2))

        self.n_scenarios = len(self.milp_objects)
        self.probs = np.full(self.n_scenarios, 1.0 / self.n_scenarios)
        print(f"  Prepared {self.n_scenarios} scenarios  (3×3×3, 3 stages of uncertainty)")
    ...
    def build(self):
        ...
        # H_set: union across all 27 scenarios — use the EARLIEST possible harvest time
        ...
        # Stage 0 — 1 node: all 27 scenarios share one plan
        _add_nac(stage0_set, lambda _: 0)
        # Stage 1 — 3 nodes: scenarios sharing t1 agree on months 20–39
        _add_nac(stage1_set, lambda i: self.scenario_bundle[i])
        # Stage 2 — 9 nodes: scenarios sharing (t1,t2) agree on months 40–59
    ...
    def print_results(self):
        ...
        normal_sc = next(
            (i for i, nm in enumerate(self.scenario_names)
             if nm == "s1_normal__s2_normal__s3_normal"), 0)
        ...
        _print_stage("Stage-0 Decisions  (months 0-19, all-normal path)",
                     self.stage0_months, normal_sc)
        _print_stage("Stage-1 Decisions  (months 20-39, all-normal path: t1=normal)",
                     self.stage1_months, normal_sc)
        _print_stage("Stage-2 Decisions  (months 40-59, all-normal scenario: t1=t2=t3=normal)",
    ...
    def plot(self, output_file: str = "stats/de_solution.png"):
        ...
        def _draw_timeline(ax, sc, title, shade_color):
            ...
            for sb, sl in [(20, "S1"), (40, "S2")]:
                ax.axvline(sb, color="gray", ls="--", lw=0.8, alpha=0.6)
                ax.text(sb + 0.4, n_units - 0.1, sl,
                        fontsize=6, color="gray", va="top")
            ax.axvspan(s2_start, T, alpha=0.07, color=shade_color, zorder=0)
            ax.text((s2_start + T) / 2, n_units - 0.15, "stage-2",
        ...
        axes[1, 2].text(
            ...
            f"Scenarios  : {self.n_scenarios}  (3³, 3 stages of uncertainty)\n"
            ...
            f"--- Scenario Tree ---\n"
            f"Stage-0: 1 node   × 27 scenarios\n"
            f"Stage-1: 3 nodes  ×  9 scenarios\n"
            f"Stage-2: 9 nodes  ×  3 scenarios",
...
def export_decision_tree(de: "DeterministicEquivalent",
    ...
    for sc_idx, name in enumerate(de.scenario_names):
        t1, t2, t3 = _parse_sname(name)
        nk0 = "s0"
        nk1 = f"s1_{t1}"
        nk2 = f"s2_{t1}_{t2}"
        sc_nodes.append({"s0": nk0, "s1": nk1, "s2": nk2})
        for nk in (nk0, nk1, nk2):
    ...
    _stage_order = {"Stage0": 0, "Stage1": 1, "Stage2": 2}
            ...
            for t in ref.Tset:
                if round(de._stk[sc_idx, u, t].X) >= 1:
                    ...
                    playbook_rows.append({
                        "scenario_id": sc_idx, "t1": t1, "t2": t2, "t3": t3,
    ...
    guide_rows = [
        {"Sheet": "Tree_Plan",
         "What it shows": "The complete 13-node decision tree.  "
                          "Stage 0 (1 node) → Stage 1 (3 nodes) → Stage 2 (9 nodes). "
                          "Stage-0 decisions are committed before any uncertainty resolves.",
         "How to use": "Read Stage 0 first. After month 19, observe t1 and jump to your Stage-1 branch. "
                       "After month 39, observe t2 and jump to your Stage-2 branch."},
        {"Sheet": "Scenario_Lookup",
         "What it shows": "27 rows — one per scenario with node keys and decision counts.",
         "How to use": "Find your scenario by t1/t2/t3 to identify relevant Tree_Plan sections."},
        {"Sheet": "Scenario_Playbook",
         "What it shows": "Full chronological action list for every scenario across all 3 stages.",
    ...
    _write_stage_banner(ws_tp, row,
                        "STAGE 0  —  Months 0–19",
                        "Committed before any uncertainty resolves.  Identical for ALL 27 scenarios.")
    row += 1
    ...
    for t1 in labels:
        ...
        _write_node_banner(ws_tp, row,
                           f"IF  t1 = {t1.upper()}",
                           f"{n_scens} of 27 scenarios follow this branch")
        row += 1
        _write_col_headers(ws_tp, row); row += 1
