...
class BinaryProgressiveHedging:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-subproblem time limit (seconds).  None = unlimited (the
        # original behaviour).  When set, Gurobi returns its best feasible
        # solution at the deadline; PH naturally accepts suboptimal
        # subproblem solutions, so this caps the wallclock pathology where
        # one penalty-deformed scenario MIP hangs the entire iteration.
        subproblem_time_limit: float = None,
        ...
        # ─── Macro-block length for the S_bad survival rule ────────────────
        # Governs how many consecutive SP stages constitute one "biological
        # block" for the good-after-good elevated-mortality rule.
        #   macro_block_len=1 (default): each stage is its own block —
        #       S_bad triggers whenever two adjacent stages are both 'good'.
        #       Correct for 30M/45M/60M where each stage = one 15-month block.
        #   macro_block_len=2: pairs of stages form one biological block —
        #       S_bad only triggers when TWO consecutive PAIRS of stages are
        #       all 'good', i.e. label_tuple must be ('good','good','good','good').
        #       Use this for 30M_81 (7+8+7+8 months; stages 0-1 share the
        #       same 15-month temperature block, as do stages 2-3).
        macro_block_len: int = 1,
    ):
        ...
        self.macro_block_len       = int(macro_block_len)
    ...
    def build(self):
        ...
        for label_tuple in self._label_tuples:
            ...
            for i, (sl, months) in enumerate(blocks):
                # Determine the "previous macro-block label" for the S_bad rule.
                #
                # With macro_block_len=1 (default) every stage is its own
                # biological block → same behaviour as before: S_bad fires
                # whenever two adjacent stages are both 'good'.
                #
                # With macro_block_len=2 (30M_81) stages 0-1 form block 0 and
                # stages 2-3 form block 1.  S_bad fires for EVERY stage in
                # block k > 0 when block k-1 was entirely 'good'.  Since all
                # stages within a block share the same label, this means
                # S_bad fires in stages 2 AND 3 only when ALL FOUR labels
                # are 'good' (the only case where block 0 is 'good').
                #
                # The prefix block (if any) is index i=0 in `blocks`; the
                # first stochastic stage starts at prefix_offset.
                prefix_offset = 1 if self.prefix_months else 0
                stage_idx = i - prefix_offset   # -1 for prefix, 0.. for stochastic
                mb_idx    = max(stage_idx, 0) // self.macro_block_len

                if stage_idx < 0 or mb_idx == 0:
                    # Prefix or first macro-block: no preceding block → no elevation.
                    prev_mb_label = None
                else:
                    # Look up the label of the first stage in the previous
                    # macro-block (all stages there share the same label).
                    prev_mb_start = (mb_idx - 1) * self.macro_block_len + prefix_offset
                    prev_mb_label = (blocks[prev_mb_start][0]
                                     if prev_mb_start < len(blocks) else None)

                surv = self._stage_surv(sl, prev_mb_label, self.S_normal, self.S_bad)
                for m in months:
                    S_t[m] = surv
        ...
        while k < K and not converged:
            ...
            def _ph_solve(s, _k=k):
                ...
                model.Params.TimeLimit = (
                    GRB.INFINITY if self.subproblem_time_limit is None
                    else float(self.subproblem_time_limit)
                )
                vidx   = self.scenario_vidx[s]
                vars_s = self.var_cache_flat[s]
                for _, (vi, var) in enumerate(zip(vidx, vars_s)):
                ...
                if model.SolCount == 0:
                    ...
                    # Same time-limit policy applies to the rescue solve
                    model.Params.TimeLimit = (
                        GRB.INFINITY if self.subproblem_time_limit is None
                        else float(self.subproblem_time_limit)
                    )
